...
$(function(){
    ...
    context: document.body
        ...
        $('#urlRedirect').val(e.urlRedirect);
        $('#urlImage').val(e.urlImage);
        $('#codCupom').val(e.codCupom);
        $("#tipo").val(e.tipo);
        var campanhaCriativo = e.campanha_id;
        $.ajax({
          ...
          context: document.body
              ...
              $(e).each(function (index, item) {                
                  ...
                  var nome = item.nome;
                  if(campanhaid == campanhaCriativo){
                    var selected = "selected";
                  }else{
                    var selected = "";
                  }
                  $('#campanha_id').append('<option value="'+ campanhaid +'" '+selected+' >'+ nome +'</option>')            
              });
          }).fail(function(e){
              console.log(e);
    ...
    $( "#formPut" ).submit(function( event ) {
        ...
        $.ajax({
            ...
            window.location.href = "{{url('criativos/'.$criativo)}}";
